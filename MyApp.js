...
class MyApp  {
    ...
    initCameras() {
        ...
        // Create a perspective camera focused on the paintings
        const perspectivePainting = new THREE.PerspectiveCamera( 75, aspect, 0.1, 1000 )
        perspectivePainting.position.set( 0, 5, -4 );
        this.perspectivePaintingTarget = new THREE.Vector3( 0, 5, -10 );
        perspectivePainting.lookAt( this.perspectivePaintingTarget );
        this.cameras[ 'Paintings' ] = perspectivePainting;

        // Create a perspective camera focused on the table
        const perspectiveTable = new THREE.PerspectiveCamera( 75, aspect, 0.1, 1000 );
        perspectiveTable.position.set( 9, 6, 7 );
        this.perspectiveTableTarget = new THREE.Vector3( 0, 0, 0 );
        perspectiveTable.lookAt( this.perspectiveTableTarget );
        this.cameras[ 'Table' ] = perspectiveTable;
    ...
    updateCameraIfRequired() {
        ...
        if (this.lastCameraName !== this.activeCameraName) {
            ...
            if (this.activeCameraName === "Paintings") {
                this.controls.target.copy( this.perspectivePaintingTarget );
            } else {
                this.controls.target.copy( this.perspectiveTableTarget );
            }
